...
<%PostVO detail = (PostVO)request.getAttribute("detail");%>	
	...
	<div id='content'>
		...
		<!-- 제목 div 시작 -->
			<div style='height:5vh; width: 90.2%; border: 1px solid black;'>
				<div style="float: left;width: 70%;"><a>タイトル:<%=detail.getPostTitle() %></a></div>
				<div style="float: left;width: 29%;" align="right"><a>作成者: <%=detail.getUserId()%>	登録日:<%=detail.getRegDate() %></a></div>				
			</div>
		<!-- 제목 div end-->
			
		<!-- photo div start-->
			<div style='height:40vh; width: 60%; border: 1px solid black; float: left; min-height: 288px;'>
				<img src="<%="/"+detail.getPostPhoto() %>" style='width: 100%;height: 40vh; min-height: 288px;'>
			</div>
		<!-- photo div end-->
		...
		<!-- informatoin div start-->
			<form id='auctionOrBuyNow' method='post'>
				...
				<div style='display:table; height:40vh; width: 30%; border: 1px solid black; float: left; min-height: 288px;'>
					<div>
					...
					<div style="display: table; padding-top: 20px; width: 100%">					
						<button id= 'showSAuctionList' type="button" style='width:45%;  float: left;'>入札者リスト</button>
						...
						<button id= 'auctionBtn' type="button" style='width:45%;  float: left;'>入札する</button>
						<button id= 'auctionCancelBtn' type="button" style='width:45%; float: left;'>入札取り消し</button>
						<button id= 'myListBtn' type="button" style='width:45%;  float: left;'>マイリスト登録</button>
						<%} %>
					</div>
				</div>
			</form>
	...
	<!-- post Content div start-->
		...
		<div style='width: 100%; padding-top: 5vh;' align="center">			
			<button type='button' style='margin-left: 0%;margin-top: 0.5vh' onclick="document.location.href='/action/S_post_showList'">戻る</button>
			<%if(session.getAttribute("userId")!=null&&(session.getAttribute("userId").equals(detail.getUserId())||session.getAttribute("userId").equals("ADMIN"))){ %>	
				<button type='button' class='postDelete' id='<%=detail.getPostNum()%>' style='margin-left: 0%;margin-top: 0.5vh'>削除</button>	
				<button type='button'  class='postModify' id='<%=detail.getPostNum()%>' style='margin-top: 0.5vh'>修正</button>	
			<%}%>
		</div>	
	...
	<tr style='width: 100%;'>
						...
						<td style='width: 10%; border: 1px solid black;'>
							...
							<%if(session.getAttribute("userId")!=null&&session.getAttribute("userId").equals(comment.getUserId())){ %>
								<button type='button'  class='commentModify' id='<%=comment.getCommentNum()%>' style='margin-bottom: 10%'>修正</button>	
								<button type='button'  class='commentDelete' id='<%=comment.getCommentNum()%>' style='margin-bottom: 10%'>削除</button>	
		...
		<tr>
			...
			<td align="center">
				<input id='commentContent' name="commentContent" type='text' style='width: 99%;' placeholder="コメントを入力" >
			</td>
			<td align="center">
				<button type='submit' formaction="/action/S_post_createComment" formmethod="post" >登録</button>		
			</td>
		</tr>	
				</table>				
		</form>
