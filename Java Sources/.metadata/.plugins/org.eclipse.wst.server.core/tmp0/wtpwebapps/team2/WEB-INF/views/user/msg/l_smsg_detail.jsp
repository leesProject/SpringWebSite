	...
	<div id='content'>		
		...
		$('#showSMsgList').click(function(){
			...
			<form id='msgReplyOrDelete'>
				...
				<button id='replyBtn' type='button'>送り直す</button>
		...
		<script>
			$('#replyBtn').click(function(){
				document.location.href='/jsp/user/msg/l_msg_create.jsp?receiverId='+encodeURIComponent('${msg.receiverId}')+'&pageNum='+pageNum;
			})
			$('#returnBtn').click(function(){
				document.location.href='/action/Msg_showSMsgList?pageNum='+pageNum;
			})
			$('#deleteBtn').click(function(){
				if(confirm("このメールを削除しますか。")){
					$('#pageNum').val(pageNum);
					$('#msgReplyOrDelete').attr('method','post');
					$('#msgReplyOrDelete').attr('action','/action/Msg_deleteSMsg');
